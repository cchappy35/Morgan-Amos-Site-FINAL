...
## Redirect loop — fixed 2026-08-13

The first deploy returned `ERR_TOO_MANY_REDIRECTS` on every page. Cause: the
inherited `_redirects` file mapped clean paths back to their files with a
`200` status —
...
— but Cloudflare Pages **already** serves `buy.html` at `/buy`, and
308-redirects `/buy.html` → `/buy` on its own. The explicit rule sent `/buy`
to `/buy.html`, Pages sent it straight back, and the browser gave up.

`_redirects` now contains genuine aliases only (`/buying` → `/buy`, etc.), all
targeting extensionless paths. **Never re-add a `path → path.html` rule.**

Nothing else changed: the in-page links still say `buy.html` and Pages
normalises them to `/buy` with a single 308, which is what the canonical tags
and sitemap already point at.


## Listing feed — CORS dependency removed

The feeds live on kpdd.com. Reading them straight from the browser only works
while the brokerage keeps sending `Access-Control-Allow-Origin` for this
site's origin — outside our control, and its failure mode is the listings
section quietly showing its "call Morgan" panel.

`functions/feed/[name].js` is a Cloudflare Pages Function that fetches the RSS
server-side and serves it from our own origin at `/feed/active`,
`/feed/pending`, `/feed/closed`, `/feed/reviews`. The browser never makes a
cross-origin request, so CORS cannot break the section. Feed names are
allowlisted, the response is cached 120s at the edge, and no secrets are
involved.

`feeds.js` tries the proxy first and falls back to the direct kpdd.com URL, so
the site degrades to its previous behaviour on any host without Functions
(local file preview, a plain static host).

Still no build step: Pages picks up `functions/` automatically. Build command
stays empty, output directory stays `/`.
